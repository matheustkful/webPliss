...
import { createLocalPreview, downloadProject } from '../services/local-preview'
...
export function StraightStairPage(){
  ...
  useEffect(()=>{saveProjects(projects);if(!activeId&&projects[0])setActiveId(projects[0].id);if(activeId&&!projects.some(project=>project.id===activeId)&&projects[0])setActiveId(projects[0].id)},[projects,activeId])

  const updateInput=(name:string,value:number|string)=>{if(!activeProject)return;setProjects(current=>current.map(project=>project.id===activeProject.id?{...project,input:{...project.input,[name]:value},updatedAt:new Date().toISOString()}:project));setPreview(null)}
  ...
  const addProject=(event:FormEvent)=>{event.preventDefault();const name=newName.trim();if(!name){setError('Informe um nome para o projeto.');return}const project=createProject(name,newKind);setProjects(current=>[project,...current]);setActiveId(project.id);setNewName('');setNewKind('UmLance');setModalOpen(false);setSidebarOpen(false);setError('')}
  const removeProject=(id:string)=>{const project=projects.find(item=>item.id===id);if(!project||projects.length===1)return;if(!window.confirm(`Excluir o projeto “${project.name}”?`))return;setProjects(current=>current.filter(item=>item.id!==id));setPreview(null);if(id===activeProject?.id)setActiveId(projects.find(item=>item.id!==id)?.id??'')}
  const saveCurrentProject=()=>{if(activeProject)downloadProject(activeProject)}
  ...
  return <div className="app-shell">
    ...
    <div className="content-shell"><ProjectSidebar projects={projects} activeId={activeProject.id} open={sidebarOpen} onSelect={setActiveId} onCreate={()=>{setModalOpen(true);setError('')}} onDelete={removeProject} onClose={()=>setSidebarOpen(false)}/><main id="top" className="main-content">
